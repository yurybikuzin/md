...
Строки `drop(ptr);` и `*ptr = 12;` конфликтуют за доступ к переменной `ptr`. Поэтому удаление одной из этих строк убирает конфликт. Обычно в программе на Rust не требуется явного вызова drop. Т.к. ресурс, которым владеет переменная, удаляется при выходе переменной за пределы области видимости. 
